...
class Sale extends AbstractReporting
{
    ...
    /**
     * Retrieves today's pending orders count (total today's orders minus completed today's orders).
     */
    public function getTodayPendingOrdersCount(): int
    {
        $totalTodayOrders = $this->orderRepository
            ->resetModel()
            ->whereIn('channel_id', $this->channelIds)
            ->whereBetween('created_at', [now()->today(), now()->endOfDay()])
            ->count();

        $completedTodayOrders = $this->orderRepository
            ->resetModel()
            ->whereIn('channel_id', $this->channelIds)
            ->whereBetween('created_at', [now()->today(), now()->endOfDay()])
        ...
        return $totalTodayOrders - $completedTodayOrders;
    }

    /**
     * Retrieves completed orders count (total orders minus non-completed orders) between dates.
     */
    public function getCompletedOrdersCount($startDate, $endDate): int
    {
        $totalOrders = $this->getTotalOrders($startDate, $endDate);

        $nonCompletedOrders = $this->orderRepository
            ->resetModel()
            ->whereIn('channel_id', $this->channelIds)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('status', '<>', 'completed')
        ...
        return $totalOrders - $nonCompletedOrders;
    }

    /**
     * Retrieves completed orders progress.
     */
    public function getCompletedOrdersProgress(): array
    {
        return [
            'previous' => $previous = $this->getCompletedOrdersCount($this->lastStartDate, $this->lastEndDate),
            'current' => $current = $this->getCompletedOrdersCount($this->startDate, $this->endDate),
            'progress' => $this->getPercentageChange($previous, $current),
        ];
    }
