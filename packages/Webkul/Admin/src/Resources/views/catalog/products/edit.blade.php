<x-admin::layouts>
    <x-slot:title>
        {{ app()->getLocale() == 'th' ? 'แก้ไขสินค้า' : __('admin::app.catalog.products.edit.title') }}
    </x-slot>
        ...
        <div class="sticky top-[62px] z-[9999] grid gap-2.5 border-b border-gray-200 bg-white py-2 dark:border-gray-800 dark:bg-gray-950">
            <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
                ...
                <div class="flex items-center gap-x-2.5">
                    ...
                    >
                        {{ app()->getLocale() == 'th' ? 'ย้อนกลับ' : __('admin::app.account.edit.back-btn') }}
                    </a>
                    ...
                    @if (
                        ...
                        <a
                            ...
                            target="_blank"
                        >
                            {{ app()->getLocale() == 'th' ? 'ดูตัวอย่าง' : __('admin::app.catalog.products.edit.preview') }}
                        </a>
                    @endif

                    <!-- Save Button -->
                    <button class="primary-button">
                        {{ app()->getLocale() == 'th' ? 'บันทึกสินค้า' : __('admin::app.catalog.products.edit.save-btn') }}
                    </button>
                </div>
            </div>
        </div>
                            ...
                            <div class="box-shadow relative rounded bg-white p-4 dark:bg-gray-900">
                                @php
                                    $groupNames = [
                                        ...
                                        'settings'         => app()->getLocale() == 'th' ? 'ตั้งค่าสินค้า' : 'Settings',
                                        'inventories'      => app()->getLocale() == 'th' ? 'คลังสินค้า' : 'Inventories',
                                        'rma'              => app()->getLocale() == 'th' ? 'ระบบส่งคืน (RMA)' : 'RMA',
                                    ];

                                    $displayName = $groupNames[$group->code] ?? $group->name;
                                @endphp
