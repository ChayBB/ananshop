<x-shop::layouts.account>
            ...
            <x-shop::tabs>
                ...
                >
                    ...
                    <div
                        ...
                        v-pre
                    >
                        <div class="text-base font-medium flex items-center gap-x-6">
                            <div>
                            ...
                            <div class="flex items-center gap-x-2">
                                <span class="text-zinc-500">@lang('shop::app.customers.account.orders.view.status'):</span>
                                @php
                                    $status = $order->custom_status ?? 'รอดำเนินการ';
                                ...
                                @endphp
                                <span class="{{ $badgeClass }} text-sm px-2.5 py-0.5 rounded font-medium">
                                    {{ $status }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="mt-1 grid gap-2.5 text-xs text-black">
                            ...
                            @if ($order->payment->method !== 'cashondelivery' && $order->custom_status !== 'เก็บเงินปลายทาง')
                                <div class="mt-2 flex flex-col gap-2">
                                    @if ($order->slip_path)
                                        <div class="relative h-[80px] w-[80px] rounded cursor-pointer border border-zinc-200 overflow-hidden hover:opacity-90 transition-all shadow-sm" onclick="window.showSlipModal('{{ \Illuminate\Support\Facades\Storage::url($order->slip_path) }}')">
                                            <img class="h-full w-full object-cover" src="{{ \Illuminate\Support\Facades\Storage::url($order->slip_path) }}" alt="Slip">
                                        </div>
                                    @elseif ($order->custom_status === 'รอดำเนินการ')
                                        <button
                                            type="button"
                                            class="w-fit rounded-md border border-orange-400 bg-orange-50 px-2 py-1 text-xs font-medium text-orange-600 transition-all hover:bg-orange-100"
                                            onclick="window.openUploadModal()"
                                        >
                                            อัปโหลดสลิป
                                        </button>
                                    @else
                                        <span class="text-xs text-zinc-400">ยังไม่ได้อัปโหลดสลิป</span>
                                    @endif
                                </div>
                            @endif
                        </div>
                ...
                <div class="grid max-w-[200px] place-content-baseline gap-4 max-868:w-full max-868:max-w-full max-md:max-w-full max-md:gap-2">
                    ...
                    @if ($order->payment->method !== 'cashondelivery' && $order->custom_status !== 'เก็บเงินปลายทาง')
                        <div class="mt-2 flex flex-col gap-2">
                            ...
                            @if ($order->slip_path)
                                <div class="relative h-[100px] w-[100px] rounded cursor-pointer border border-zinc-200 overflow-hidden hover:opacity-90 transition-all shadow-sm" onclick="window.showSlipModal('{{ \Illuminate\Support\Facades\Storage::url($order->slip_path) }}')">
                                    <img class="h-full w-full object-cover" src="{{ \Illuminate\Support\Facades\Storage::url($order->slip_path) }}" alt="Slip">
                                </div>
                            @elseif ($order->custom_status === 'รอดำเนินการ')
                                <button
                                    type="button"
                                    class="secondary-button w-fit border-orange-400 bg-orange-50 px-3 py-1.5 text-xs font-medium text-orange-600 transition-all hover:bg-orange-100"
                                    onclick="window.openUploadModal()"
                                >
                                    อัปโหลดสลิป
                                </button>
    ...
    <!-- Upload Slip Modal -->
    <div id="uploadSlipModal" class="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/50 backdrop-blur-sm" onclick="window.closeUploadModal()">
        <div class="relative w-full max-w-md overflow-hidden rounded-lg bg-white p-6 dark:bg-gray-900 shadow-2xl border dark:border-gray-800" onclick="event.stopPropagation()">
            <div class="flex justify-between items-center pb-4 border-b dark:border-gray-800">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">อัปโหลดหลักฐานการชำระเงิน</h3>
                    <p class="text-xs text-gray-500 mt-0.5">คำสั่งซื้อ #{{ $order->increment_id }}</p>
                </div>
                <button class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 text-2xl font-semibold leading-none" onclick="window.closeUploadModal()">&times;</button>
            </div>
            
            <form action="{{ route('shop.checkout.slip.upload', $order->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 pt-4">
                @csrf
                <div>
                    ...
                    <input
                        ...
                        required
                        class="block w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-white focus:border-orange-500 focus:outline-none"
                    >
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" class="px-4 py-2 border rounded text-sm text-gray-700 dark:text-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800" onclick="window.closeUploadModal()">ยกเลิก</button>
                    <button type="submit" class="primary-button !px-4 !py-2">อัปโหลด</button>
                </div>
            </form>
        </div>
    </div>
